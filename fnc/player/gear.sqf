...
//uniform & helmet
	...
	if (groupId (group _unit) in ["SUNRAY","BULLFROG"]) then {
...
//sidearms only for seals, medic & leaders
	if ((groupId (group _unit) in ["SUNRAY","BULLFROG"]) || typeOf _unit in ["B_medic_F","B_Soldier_TL_F","B_Soldier_SL_F","B_officer_F"]) then {
	...
	if (groupId (group _unit) in ["SUNRAY","BULLFROG"]) then { //defalut seals
		_unit addGoggles "G_Lowprofile";
	} else { //default medic backpack equipment every marine
